...
### FSIM
特征相似性指数（Feature Similarity Index）
##### （1）核心用途
...
### DISTS：
基于深度学习的图像质量评价指标，基于CNN（深度图像结构与纹理相似性）
##### 1. 核心用途
...
### BRISQUE
盲图像质量指数（Blind Image Quality Index）
##### （1）核心用途
...
同样基于深度神经网络（无参考）
##### （1）**核心用途**
适用于社交媒体图像、监控视频等无参考场景，对JPEG压缩、噪声、模糊等失真类型鲁棒。  
##### （2）**核心思想**
...
##### （4）**性能表现**
在CLIVE数据集上SROCC达0.601，优于BRISQUE（0.557）和NIQE（0.477）。
...
##### （1）**核心用途**
基于CLIP模型的无参考指标，适用于AI生成图像（如GAN）、艺术创作等抽象感知评估。  
##### （2）**核心思想**：  
...
### PI
（Perceptual Index）：基于CNN
##### （1）**核心用途**
基于预训练CNN的无参考指标，适用于自然图像的美学评分和质量筛选。  
##### （2）**核心思想**
...
### MANIQA
（Multi-Attention Network for Image Quality Assessment）
##### （1）**核心用途**
针对GAN生成图像的无参考指标，解决传统方法对生成失真（如伪影、纹理不连贯）评估失效的问题。  
##### （2）**核心思想**
...
| 无参考图、生成式图像    | LPIPS      | 捕捉高层语义，适用于GAN生成图、风格迁移  |
